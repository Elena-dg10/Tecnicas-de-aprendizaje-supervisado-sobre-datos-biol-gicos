...
any(df_data == 0) # Tenemos valores iguales a cero.
# Sumamos los valores cero por columnas.
...
sapply(df_data, function(x) length(unique(x))) # Todas tienen variabilidad ya que ninguna nos da un valor de 1.
...
# Escalamos los datos y normalizamos. La función scale() sirve para ambas cosas ya que integra z-score (media 0, desviación testándar 1).
df_escalado <- scale(df_numerico)
# Lo convertimos de nuevo en dataframe ya que la función scale() devuelve un objeto matriz.
# Con cbind() devolvemos la columna de la variable predictora "diagnosis".
...
# Veremos si hay multicolinealidad antes de elegir un método de reducción de dimensiones:
# Lo hacemos con la matriz de correlación:
...
# Aplicamos entonces el modelo RIDGE con alpha = 0 y el mínimo lambda.
set.seed(1234)
...
# En el resultado vemos que hay relaciones positivas y negativas. 
#Cuanto mayor es el valor, más impacto tiene en la predicción. 
#Por ejemplo, texture3=0,454 influye más que smoothness2 = 0,018.
...
# library(ggplot2)
# 
# coefs <- as.matrix(coef(modelo_ridge))
# coefs_df <- data.frame(
#   variable = rownames(coefs),
#   coef = as.numeric(coefs)
# )
# 
# # Quitamos el intercepto para la gráfica
# coefs_df <- coefs_df[coefs_df$variable != "(Intercept)", ]
# 
# ggplot(coefs_df, aes(x = reorder(variable, abs(coef)), y = coef)) +
...
# RANDOM FOREST
# El primer odelo que vamos a probar es RANDOM FOREST porque: su objetivo es combinar un gran número de árboles de decisión en un gran bosque mediante el bagging (se basa en el concepto de Bootstrapping, el cual consiste en obtener subconjuntos de datos con remplazamiento y entrenar cada algoritmo débil con uno de estos subconjunto).
...
# Entrenamos el modelo usando la función randomForest del paquete randomForest. Especificamos un nº de árboles de 100 (ntree=100):
...
# En los resultados vemos que hemos creado un bosque con 100 árboles de decisión (clasificación).
# En cada árbol se prueban 5 variables aleatorias.
...
# En cuanto a la matriz de confusión vemos que se clasificaron correctamente 279 benignos frente a 161 malignos. 
# El modelo se equivocó en 7 casos benignos clasificados como malignos (falsos positivos).
# El modelo se volvió a equivocar en 9 casos malignos clasificados como benignos (fañsos negativos).
# La Accuracy o exactitud es del 96,5% que es muy buena, el modelo acierta casi todos los casos.
#EL IC (intervalo de confianza) es del 95% por lo que tenemos alta certeza de que es así y no se equivoca o es casualidad.
# En el PLOT se observan los errores cometidos para cada árbol de decisión. 

# El parámetro Kappa mide el acierto del modelo entre lo predicho y lo real, en comparación con lo que podría haber sido ajustado por el azar. Un valor entre 0.8-1 es muy bueno. Esto quiere decir que hay concordancia entre lo predicho y lo real.
# Tenemos buenas tasas de sensibilidad (detección acertada de positivos o benignos) y especificidad (detección buena de malignos o negativos): siendo 97% y 94% respectivamente. 
# Por último la clase positiva nos sale la B (benigno) lo cual es concordante con la realidad.


# Obtenemos las predicciones y  probabilidades de las clases, y guardamos los datos de confusionMatrix para futuras comparaciones.
# Usamos los datos de testing ya que son los que el modelo no conoce hasta el momento y vamos a ver qué tan bien consigue clasificar.
...
# Antes de hacer la matriz de confusion volvemos a convertir a factor la variable predictions_RF y diagnosis ya que al dividir los conjuntos de training y testing muchas veces se deshace (como ha ocurrido ahora):
...
confusionMatrix(predictions_RF, testData$Diagnosis)
  ...
  # Kappa vuelve a estar por encima de 0,8 sinedo 0,88 y la sensitividad y especificidad del 95% y 92% respectivamente.
  # De nuevo la clase positiva clasificada es la B de Benigno. 
  # En cuanto a la matriz 68 casos positivos reales fueron clasificados como tal, cometiendo error solo con tres de ellos. Para el caso de los malignos 39 fueron clasificados correctamente, con un error de tres casos clasificados benignos siendo malignos.
...
# DECISSION TREES
# He elegido este modelo para el análisis porque es fácilmente interpretable, captura relaciones no lineales y es fácil de visualizar. Hay que tener cuidado, según he leído, con el sobreajuste.
# Usamos la función train() del paquete "caret". Usamos rpart para indicar que es un árbol de decisión y a su vez, clasificación.
...
# Creamos el árbol de decisión y lo entrenamos usando trainControl conunas particiones o repeticiones de 10 veces. 
# Probamos 10 valores diferentes de complexity parameter (evita sobreajuste).
# Se eligió el cp = 0 porque dio la mayor precisión (accuracy = 91.89%). 
#  Por otro lado kappa, qu emide acuerdo entre predicciones y realidad, no es malo ya que tenemos valores de 0,8.
...
# Antes de hacer la matriz de confusion volvemos a convertir a factor la variable predictions y diagnosis ya que al dividir los conjuntos de training y testing muchas veces se deshace (como ha ocurrido ahora):
...
# Vemos una matriz de confusión bastante buena dond epredice 278 benignos bien, respecto a 7 mal, y 163 malignos bien con respecto a 8 mal. 
# El accuracy es muy bueno, del 96% por lo que confirma que el modelo acierta casi todos los casos. EL CI es del 95% por lo que tenemos certeza de que no fue por azar.
# El parámetro kappa es de 0,92 por lo que es un valor muy bueno, muy cercano al máximo (1). 
# De nuevo la sensitividad y especificidad son valores de más del 90%, por lo que son excelentes.
# La clase positiva B "Benigno" ha sido clasificada correctamente como tal.
...
# # De nuevo vemos que la matriz de confusión es bastante buena y tenemos un accuracy muy buena del 91% y un CI del 95%.
# Kappa vuelve a ser 0,80 y la sensitividad y especificidad del 95% y 92% respectivamente. Son valores muy buenos.
# De nuevo la clase positiva clasificada es la B de Benigno. 
# En cuanto a la matriz 68 casos positivos reales fueron clasificados como tal, cometiendo error en siete de ellos. Para el caso de los malignos 35 fueron clasificados correctamente, con un error de tres casos clasificados benignos siendo malignos.
# Estos valores nos demuestran que realmente el modelo está entendiendo y clasificando bien nuestros datos, sin apenas error.
...
# Elijo el modelo SVM de la librería caret ya que es preciso con problemas complejos y grandes diensiones. En cambio perderé en el ajuste así como en su interpretabilidad.
# Hacemos un modelo lineal ya que las clases Maligno y Benigno son fácilmente separables y así restamos complejidad al análisis y posibilidad de cometer otros errores o sobreajustar.
# En este modelo que depende de la distancia entre puntos sí que necesito escalar y centrar los datos (como nu fue el caso en el anterior).
# Si las variables están en escalas muy diferentes, puede sesgar las distancias y afectar el rendimiento.
# Probé con number = 5 en trainControl pero había mcha variación, y por lo que investigué, 10 suele ser el óptimo y más ajustado. Suelen ponerse menos pruebas de cross validation cuando no tienes recursos computacionales o tiempo para ejecutar las 10.
# En cuanto a C, que es el parámetro de penalización del error, estoy eligiendo una gran precisión porque particiono valores entre 0 y 2 20 veces. En este caso he puesto que parta no desde 0 sino de 0.0001 porque si lo hacía desde cero, el primer valor de Accuracy y Kappa era NaN. Por tanto, he probado varios valores distintos de cero, y el que más se parecía en resultados a 0 (pero evitando los NaN) era 0.0001.
...
# El valor C nos sale de 0,84 de las 20 particiones que hizo.
# La accuracy más alta ha sido de 98% y el kappa de 95%.
# Todos estos valores son bastante buenos, pero ahora veremos si el modelo es realmente capaz con los datos de testing.
...
# La accuracy (exactitud) ha subido con respecto a otros modelos sieindo del 98%.
# El CI del 95% nos da fiabilidad en las predicciones, no tratándose estas del azar.
# Kappa es más alto también que en otros modelos, con un 0,97 (97%), así como la sensitividad y especificidad del 100% y 96% respectivamente).
# De nuevo la clase positiva es la B de Benigno, clasificada correctamente.
...
# Con el conjunto testing, el modelo obtuvo una accuracy del 96.46% con un IC del 95%.
# El Kappa fue de 0.92, lo que indica una excelente concordancia entre predicciones y datos reales.
# La matriz de confusión muestra que el modelo clasificó correctamente 71 casos benignos como tal, 38 malignos correctamente encasillados como malignos, pero hubo 4 malignos fueron mal clasificados como benignos (falsos negativos).
# La sensibilidad fue 100% (detectó todos los benignos) y la especificidad fue 90.5% (detectó la mayoría de los malignos correctamente).
# La clase B fue correctamente clasificada.
...
# La matriz de confusión identificó 283 casos benignos correctamente frentre a 8 errores. Por otro lado, clasificó 162 malignos correctamente frente a 3 fallos. 
# Una accuracy del 97% con IC del 95% es un muy buen funcionamiento del modelo. 
# El kappa es el más alto hasta el momento, siendo del 94%.
# De nuevo la clase benigna B ha sido correctamente clasificada como positiva.
...
# Al meter los datos de testing, vemos que el error es también bastante bajo, siendo de 3 errores frente a 71 aciertos para los casos benignos y de 39 malignos sin error.
# Accuracy y el IC de nuevo 97% y 95% respectivamente. 
# El kappa vuelve a ser dle 94%, muy alto.
# Sensitividad de 1 es perfecta y especificidad del 92% prácticaemnte también. 
# Clase B predicha correctamente.
...
# COMPARACIÓN DE LOS MODELOS : CURVAS ROC #
...
# Dibujo todas las curvas roc juntas:
...
# Preparamos las probabilidades para comparación:Probabilidades guardadas durante la práctica:
# Para Decision Tree:
